...
def generate_bmp():
    ...
    try:
        ...
        # Get last 3 rows (assumes headers in Row 1)
    ...
    draw.line([640, 155, 690, 155], fill=0, width=6) # Horizontal
    draw.line([665, 130, 665, 180], fill=0, width=6) # Vertical

    # 3. RECENT ACTIVITY
    draw.line([50, 285, 750, 285], fill=0, width=2)
    draw.text((60, 305), "Recent Activity", fill=0, font=font_activity)
    
    y_off = 350
    for tx in recent_tx:
        # Assumes your Sheet has columns: Date, Type, Amount, Description
        t_date = str(tx.get('Date', ''))[:10] # Clean up date string
        t_type = tx.get('Type', '+')
...
if st.button('🔄 Sync Data & Generate Card'):
    with st.spinner('Calculating allowance and drawing card...'):
        generated_img = generate_bmp()
        
        if generated_img:
            # Display high-res preview
            ...
            st.success("Success! Open this file in the Waveshare App to tap your card.")
